...
from stage2.main import Stage2Pipeline
from stage3.main import Stage3Pipeline
...
async def run_stage3(args):
    ...
    try:
        video_data_filepath, video_prompts_filepath = await pipeline.run_full_stage3(
            args.storyboard
        )
...
async def run_full_pipeline(args):
    ...
    stage2_args = argparse.Namespace(
        conversation=conversation_file, output_dir=args.output_dir
    )
    ...
    stage3_args = argparse.Namespace(
        storyboard=storyboard_file, output_dir=args.output_dir
    )
...
Examples:
  ...
  python main.py stage1 --participants-config participants_config.json --director-config director_config.json
    ...
    stage1_parser = subparsers.add_parser(
        "stage1", help="Run Stage 1: Conversation generation"
    )
    stage1_parser.add_argument(
        "--max-turns", type=int, default=5, help="Maximum conversation turns"
    )
    stage1_parser.add_argument(
        "--participants-config",
        default="config/participants_config.json",
        help="Path to custom configuration JSON file",
    )
    stage1_parser.add_argument(
        "--director-config",
        default="config/director_config.json",
        help="Path to custom configuration JSON file",
    )
    stage1_parser.add_argument(
    ...
    stage2_parser = subparsers.add_parser(
        "stage2", help="Run Stage 2: Storyboard generation"
    )
    stage2_parser.add_argument(
        "--conversation", required=True, help="Path to conversation JSON file"
    )
    stage2_parser.add_argument(
        "--output-dir", default="pipeline_output", help="Output directory"
    )
    ...
    stage3_parser = subparsers.add_parser(
        "stage3", help="Run Stage 3: Video generation"
    )
    stage3_parser.add_argument(
        "--storyboard", required=True, help="Path to storyboard JSON file"
    )
    stage3_parser.add_argument(
        "--output-dir", default="pipeline_output", help="Output directory"
    )
